...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = rest.map(lambda evaluate: scheme_eval(evaluate, env))
...
def complete_eval(val):
    """If VAL is a Thunk, returns the result of evaluating its expression
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            if isinstance(procedure, PrimitiveProcedure):
                #if something to do with tail context:
                #call scheme_eval with tail call
                #else:
                #result = procedure.apply(args, env) #use of thunk???
                return "how does one apply thunk to this"
                ...
                #TAIL CALL
                #-last body sub exprression in a lambda
